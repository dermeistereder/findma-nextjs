...
export default async function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {/* Austrian first — Kauf Österreichisch */}
      {green.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            <h2 className="text-sm font-semibold text-green-800 bg-green-50 border border-green-200 rounded-lg px-4 py-2 inline-flex items-center gap-2">
              🇦🇹 Österreich <span className="font-normal text-green-600">({green.length})</span>
            </h2>
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-green-800 bg-green-100 border border-green-300 rounded-full px-3 py-1">
              🛒 Kauf Österreichisch
            </span>
          </div>
